...
from accounts.models import Booking, Destination, BookingStatus, TravelGuide, PaymentStatus, TopDestination, TopDestinationBooking, BookingNotification
    ...
    def form_valid(self, form):
        ...
        if travel_guide_id:
            try:
                ...
                form.instance.travel_guide = travel_guide
...
class MyBookingsView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return Booking.objects.select_related('package', 'travel_guide').filter(user=self.request.user)
...
class AdminBookingListView(UserPassesTestMixin, ListView):
    ...
    def get_queryset(self):
        return Booking.objects.select_related('package').all().order_by('-created_at')
...
def confirm_booking(request, destination_id):
    if request.method == 'POST':
        ...
        try:
            travel_guide = get_object_or_404(TravelGuide, id=travel_guide_id)
        except (ValueError, TravelGuide.DoesNotExist):
            messages.error(request, 'Invalid travel guide selected.')
        ...
        booking = Booking.objects.create(
            ...
            travel_guide=travel_guide  # Assign the travel guide
        )
        
        # Create notification
        BookingNotification.objects.create(
            booking=booking,
            message=f"New booking request for {destination.name} with travel guide {travel_guide.name}"
        )
